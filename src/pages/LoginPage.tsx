  ...
  const handleTelegramLogin = async () => {
    ...
    try {
      ...
      for (let i = 0; i < 60; i++) {
        ...
        try {
          const result = await telegramCheck(authCode)
          if (result) {
            localStorage.setItem('auth', JSON.stringify({ token: result.token, telegramId: result.telegram_id }))
            setAuth(result.token, result.telegram_id)
            return
          }
        } catch {
          // 404 = code expired, stop polling
          break
        }
      }
      setInfo('')
    ...
    <div className="min-h-screen bg-surface-950 relative">
      ...
      <div className="max-w-sm w-full text-center space-y-8">
        ...
        <div className="glass-card p-6 space-y-4">
          ...
          {info && screen === 'login' && <p className="text-blue-400 text-xs text-center">{info}</p>}
        </div>
